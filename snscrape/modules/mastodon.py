...
class _MastodonCommonScraper(snscrape.base.Scraper):
	...
	def _rate_limited_get(self, *args, **kwargs):
		diff = time.time() - self._lastRequest
		if diff < 3:
			time.sleep(3 - diff)
	...
	def _entries_to_items(self, entries, url):
		for entry in entries:
			...
			attachmentsDiv = entry.find('div', class_ = 'attachment-list')
			mediaGalleryDiv = entry.find('div', attrs = {'data-component': 'MediaGallery'})
			if attachmentsDiv:
				attachments = []
			...
			elif mediaGalleryDiv: # Before 2.7.0 (https://github.com/mastodon/mastodon/issues/6714)
				...
				if attachmentsDiv: # Before 2.3.0 (commit 2bbf987a)
					attachments = []
					for a in attachmentsDiv.find_all('a'):
			...
			pollDiv = entry.find('div', attrs = {'data-component': 'Poll'})
			if pollDiv:
