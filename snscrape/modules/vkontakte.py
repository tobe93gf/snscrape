...
class VKontakteUserScraper(snscrape.base.Scraper):
	...
	def _date_span_to_date(self, dateSpan):
		...
		match = _datePattern.match(dateSpan.text)
		if match:
		...
		match = re.match(r'^(?P<day>\d+)\s+(?P<month>' + '|'.join(_months) + r')\s+(?P<year>\d{4})$', dateSpan.text)
		if match:
			# Date only
	...
	def _post_div_to_item(self, post, isCopy = False):
		...
		h = self._away_a_to_url(a)
		outlinks = [h for a in textDiv.find_all('a') if h] if textDiv else []
