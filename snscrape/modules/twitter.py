...
class _TwitterAPIScraper(snscrape.base.Scraper):
	...
	def _make_tweet(self, tweet, user, retweetedTweet = None, quotedTweet = None, card = None):
			...
			for medium in tweet['extended_entities']['media']:
				if medium['type'] == 'photo':
					...
					if medium['type'] == 'video':
						...
						ext = medium.get('ext')
						mediaStats = ext['mediaStats']
						r = mediaStats['r']
						if ext and mediaStats and isinstance(r, dict) and 'ok' in r and isinstance(r['ok'], dict):
							mKwargs['views'] = int(r['ok']['viewCount'])
						else:
							mediaStats = medium.get('mediaStats')
							if mediaStats:
		...
		inReplyToTweetId = tweet.get('in_reply_to_status_id_str')
		if inReplyToTweetId:
		...
		if tweet.get('place'):
			...
			if 'coordinates' not in kwargs and tweet['place'].get('bounding_box') and coords and coords[0] and len(coords[0][0]) == 2:
...
class TwitterUserScraper(TwitterSearchScraper):
	...
	def _get_entity(self):
		...
		label = None
		labelO = user['affiliates_highlighted_label'].get('label')
		if labelO:
